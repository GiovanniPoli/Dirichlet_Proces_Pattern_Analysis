...
for(data.seed in 1:15){
  
  
  FileName = paste("01052021_Unscaled_DPMM_Scenario_II_seed", data.seed, sep = "_")
  ...
  rho.j2 = paste0("grp.", 1:25) 
  names(rho.j2) = measure
  
  Theta_Star = list()
  
  for(j in 1:24){
    m = c(rmvnorm(1, mean = c(0,0,0), sigma = diag(c(1,1,1))))
    names(m) = loc
    Theta_Star[[paste0("grp.",j)]] = m
  }
  ...
  rho_start    =c(rep("grp.1", J))
  
  names(rho_start) =  colnames(Data.Simulated[2:26])
  ...
  for(iter in 1:Max.iter){ 
    ...
    for(h in names(STATE$theta.star)){
      ...
      M.deltas.j = matrix(rep(STATE$delta[Y3j.2.$ID],3), ncol=3, nrow=Nj, byrow = FALSE)
  ...
  FileName = paste("01052021_Unscaled_Parametric_Scenario_II_seed", data.seed, sep = "_")
  
  ### Starting Values                 -----
  
  mu_start     = t(apply(na.omit(Data.Simulated[Data.Simulated$State=="healthy",2:26]), 2, function(x)tapply(x, na.omit(Data.Simulated[Data.Simulated$State=="healthy",])$Location, mean)))
  
  Thetas = matrix(0, nrow = J, ncol=L)
  ...
  for(iter in 1:Max.iter){ 
    ...
    for(j in rownames(STATE$betas)){
      ...
      M.deltas.j = matrix(rep(STATE$delta[ID.j],3), ncol=3, nrow=Nj, byrow = FALSE)
