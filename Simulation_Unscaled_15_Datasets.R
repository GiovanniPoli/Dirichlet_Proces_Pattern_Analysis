...
for(data.seed in 1:15){
    ...
    FileName = paste("v2_08042021_Unscaled_DPMM_sigma2_High_High_seed", data.seed, sep = "_")
    ...
    for(iter in 1:Max.iter){ 
      ...
      #### STEP XIi - M                        ----- 
      phi = rbeta(1, shape1 = STATE$M + 1, shape2 = J) # 27 n citochine
      odds = (PRIORS$M["shape"] + Hi -1)/(J*(PRIORS$M["rate"]-log(phi)))  
      pi_G1 = odds/(1+odds)
  ...
  FileName = paste("v2_08042021_Unscaled_Parametric_sigma2_High_High", data.seed, sep = "_")
